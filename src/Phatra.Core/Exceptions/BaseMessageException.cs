...
namespace Phatra.Core.Exceptions
{
    ...
    {
        private bool _loadMessageByLanguageFile = false;

        protected virtual bool LoadMessageByLanguageFile
        {
            get { return this._loadMessageByLanguageFile; }
            set { this._loadMessageByLanguageFile = value; }
        }
        ...
        {
            string xmlPath = this.XmlPath;
            if (this.LoadMessageByLanguageFile && Thread.CurrentThread.CurrentUICulture.Name != string.Empty)
            {
                xmlPath = Path.Combine(Path.GetDirectoryName(this.XmlPath), Path.GetFileNameWithoutExtension(this.XmlPath) + "." + Thread.CurrentThread.CurrentUICulture.TwoLetterISOLanguageName + Path.GetExtension(this.XmlPath));
            }
            ...
            if (additionalContents != null)
            {
                if (message == string.Empty)
                {
                    ...
                    {
                        message = additionalContents[0].ToString();
                    }
                }
                else
                {
                    for (int i = 0; i < additionalContents.Length; i++)
                    {
                        ...
                        otherMessage = otherMessage.Replace("{" + i.ToString() + "}", additionalContents[i]);
                    }
                }
            }
            this.ErrorMessage = message;
            this.OtherErrorMessage = otherMessage;
        }
    }
}
