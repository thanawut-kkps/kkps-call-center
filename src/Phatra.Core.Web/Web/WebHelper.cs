...
namespace Phatra.Core.Web
{
    ...
    public partial class WebHelper : IWebHelper
    {
        ...
        public virtual string GetThisPageUrl(bool includeQueryString, bool useSsl)
        {
            ...
            {
                string hostLocation = GetHostLocation(useSsl);
                if (hostLocation.EndsWith("/"))
                    hostLocation = hostLocation.Substring(0, hostLocation.Length - 1);
                url = hostLocation + _httpContext.Request.RawUrl;
            }
            else
            {
        ...
        public virtual void RestartAppDomain(bool makeRedirect = false, string redirectUrl = "")
        {
            ...
            else
            {
                ...
                if (!success)
                {
                    throw new Exception("Website needs to be restarted due to a configuration change, but was unable to do so." + Environment.NewLine +
        ...
        public virtual string GetLocation(bool useSsl)
        {
            ...
            string result = GetHostLocation(useSsl);
            if (result.EndsWith("/"))
